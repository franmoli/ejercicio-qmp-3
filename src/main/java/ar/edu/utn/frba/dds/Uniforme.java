...
public class Uniforme {
  ...
  public Uniforme(Prenda superior, Prenda inferior, Prenda calzado) {
    ...
    if (superior.tipo.getTipo() != CategoriaPrenda.PARTE_SUPERIOR) {
      throw new IllegalArgumentException("Prenda superior incorrecta");
    }
    this.superior = superior;

    if (inferior.tipo.getTipo() != CategoriaPrenda.PARTE_INFERIOR) {
      throw new IllegalArgumentException("Prenda inferior incorrecta");
    }
    this.inferior = inferior;

    if (calzado.tipo.getTipo() != CategoriaPrenda.CALZADO) {
